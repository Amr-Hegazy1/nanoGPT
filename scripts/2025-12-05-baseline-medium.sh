...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
    ...
    cbrun srund -t ${slurm_cluster} -x "-p ${slurm_partition} -c 4 -J ${JOB_NAME} -o ${LOG_DIR}/slurm-%j.out --time 48:00:00 --wckey sparse_scaling_law --gres=gpu:8 --nodes=1 --tasks-per-node=8 --exclusive" -e "bash -lc 'source ${SCRIPT_PATH} && run_job'"
fi