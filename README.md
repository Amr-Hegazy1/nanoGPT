...
*   **Multiple Shared Blocks:** Set `--recurrent_shared_num_blocks=<k>` (default `1`) to run `k` shared blocks sequentially on each recurrent step. With depth `D`, the execution order is `[blk0, blk1, ..., blk{k-1}]` repeated `D` times, giving `D * k` total block applications. Combine with `--share_parameters_across_layers=True` (and typically `--recurrent_shared_weights=True`).

*   **Prelude Injection:** Use `--recurrent_prelude_injection=True` to inject the output of the prelude block into each recurrent block. Choose how it mixes in with `--recurrent_prelude_injection_mode=add|concat` (default `add`) and, when running multiple shared blocks, control the scope with `--recurrent_prelude_injection_first_block_only=True` to only inject on the first block per recurrent step. The `concat` mode concatenates the prelude activations with the recurrent input and projects them back to the model width, mirroring the `add`/`concat` options from the `recurrent-pretraining` repository. This option requires `--fixed_edge_blocks=True` to be set.
